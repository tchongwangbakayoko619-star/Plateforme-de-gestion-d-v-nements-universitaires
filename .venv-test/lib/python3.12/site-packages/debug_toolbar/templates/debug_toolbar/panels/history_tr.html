...
<tr class="{% if request_id == current_request_id %}djdt-highlighted{% endif %}"
    id="historyMain_{{ request_id }}"
    data-request-id="{{ request_id }}">
  <td>{{ history_context.history_stats.time|escape }}</td>
  <td>
  ...
  <td>
    <button type="button"
            ...
            data-toggle-name="historyMain"
            data-toggle-id="{{ request_id }}">+</button>
