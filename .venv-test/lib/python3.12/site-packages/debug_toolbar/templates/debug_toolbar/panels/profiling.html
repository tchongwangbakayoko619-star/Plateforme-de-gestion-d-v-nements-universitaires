...
<table>
  ...
  <tbody>
    {% for call in func_list %}
      <tr class="djdt-profile-row {% if call.is_project_func %}djdt-highlighted {% endif %} {% for parent_id in call.parent_ids %} djToggleDetails_{{ parent_id }}{% endfor %}"
          ...
          <div data-djdt-styles="paddingLeft:{{ call.indent }}px">
            {% if call.has_subfuncs %}
              <button type="button"
                      ...
                      data-toggle-name="profilingMain"
                      data-toggle-id="{{ call.id }}">-</button>
            {% else %}
